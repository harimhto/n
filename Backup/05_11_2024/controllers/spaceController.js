...
exports.updateSpace = (req, res) => {
    ...
    const account_type = req.body.type;
    const space_description = req.body.description;
    const space_privacy = req.body.privacy;

    if (user_id && space_id && account_type == 'space') {
        ...
        connection.query(updateSpaceQuery, [space_name, space_description, space_privacy, space_id], (error, results) => {
            ...
            if (results.affectedRows > 0) {
                ...
                // callsocket(account_type,space_name,space_description,space_id);
                status = 'Space project updated successfully';
                return res.status(200).json({ success, status });
            } else {
                return res.status(404).json({ success, message: 'Space project not found or not authorized' });
            }
        });
    ...
    if (user_id && space_id && account_type == 'project') {

        const updateSpaceQuery2 = `
            UPDATE task_space_projects 
            SET project_name = ?
            WHERE id = ?`;
        
        connection.query(updateSpaceQuery2, [space_name, space_id], (error, results2) => {
            if (error) {
                console.error('Error executing query:', error);
                return res.status(500).json({ success, message: 'Database error', error });
            }
...
function callsocket(account_type,space_name,space_description,space_id){
       try {
        ...
        const spaceDesc = space_description;
        const updateId = space_id;

        io.emit('updateSpaceProject', { type, spaceName, spaceDesc, updateId }); // Emit event

    
    } catch (error) {
        console.error("Socket.IO instance is undefined,  Sapce project update page event not emitted.", error);
    }
}