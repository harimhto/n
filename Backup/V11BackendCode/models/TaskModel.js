...
static listTasks(filters, spaceId, projectId, callback) {
    ...
    this.getspace(userId, (err, spaces) => {
        ...
        let query = `
            SELECT tasklist.*, task_space.space_name as spaceNameTask, task_space_projects.project_name as projectNameTask, taskstatus.status_name, taskstatus.color, 
                   ts2.name AS assignee_name, taskpriority.color AS prColor, 
                   taskpriority.priority_name
            FROM tasklist
            LEFT JOIN taskstatus ON taskstatus.id = tasklist.status
            LEFT JOIN task_space ON CAST(task_space.id AS CHAR(255)) = CAST(tasklist.space_id AS CHAR(255))
            LEFT JOIN task_space_projects ON CAST(task_space_projects.id AS CHAR(255)) = CAST(tasklist.project_id AS CHAR(255))
            LEFT JOIN taskpriority ON taskpriority.id = tasklist.priority
            LEFT JOIN users ON FIND_IN_SET(users.id, tasklist.assignee) > 0
            LEFT JOIN users ts2 ON ts2.id = tasklist.user_id
            WHERE tasklist.taskshow = 0
...
if (filters.filterData['value2']) {
    ...
    queryParams.push('24', '24'); // Push userId for both placeholders
...
static listTasksAfterInsert(lastInsertedId, callback) {
    ...
    let query = ` SELECT tasklist.*, task_space.space_name as spaceNameTask, task_space_projects.project_name as projectNameTask, taskstatus.status_name, taskstatus.color, 
                   ts2.name AS assignee_name, taskpriority.color AS prColor, 
                   taskpriority.priority_name
            FROM tasklist
            LEFT JOIN taskstatus ON taskstatus.id = tasklist.status
            ...
            LEFT JOIN users ON FIND_IN_SET(users.id, tasklist.assignee) > 0
            LEFT JOIN users ts2 ON ts2.id = tasklist.user_id
            WHERE tasklist.taskshow = 0 AND tasklist.id = ?
        `;
