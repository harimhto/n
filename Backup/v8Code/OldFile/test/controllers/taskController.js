...
exports.storeTask = (req, res) => {
    ...
    if (spaceId && issueName && userId) {
        ...
        const storeTaskQuery = `
            INSERT INTO tasklist (user_id, task_category, task_id, owner_id, status, project_id, space_id, description, assignee, due_date, priority, estimateminutes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;

        db.query(storeTaskQuery, [userId, issueName, taskId, userId, status, projectId, spaceId, description, jsonAssignee, dueDate, getpriority, estimate], (err, result) => {
            if (err) {
