...
if (pa_taskID != undefined || pa_taskID != null || pa_taskID != '') {
    ...
    if (spaceId && issueName && userId) {
        ...
        const storeTaskQuery = `
            INSERT INTO tasklist (user_id, task_category, task_id, owner_id, status, project_id, space_id, description, assignee, due_date, priority, estimateminutes, parentId, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;

        db.query(storeTaskQuery, [userId, issueName, taskId, userId, finalStatus, projectId, spaceId, description, jsonAssignee, dueDate, finalPriority, estimate, pa_taskID, getdVal], (err, result) => {
            if (err) {
...
exports.updatetaskinfo = (req, res) => {
    ...
    db.query(query, [task_id], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            const updateQuery = `UPDATE tasklist SET ${column} = ?, update_user_id = ?, update_user_id = ? WHERE id = ?`;
            db.query(updateQuery, [value, user_id, user_id, task_id], (err) => {
...
return res.status(200).json({ success: true, status: '200' });   
    ...
    const sql3 = `
              SELECT name, color FROM users WHERE id IN (?) `;
